...
async function getFollowers(username: string) {
  ...
  const snap = await followsRef.where('target', '==', username).get();
  const followerUsernames = snap.docs.map(d => d.data().follower as string);
  if (followerUsernames.length === 0) return [] as any[];
  // Fetch user docs in batches
  const usersRef = firestoreAdmin.collection('users');
  const results: any[] = [];
  for (const u of followerUsernames) {
...
export default async function FollowersPage({ params }: { params: { username: string } }) {
  const list = await getFollowers(params.username);
  return (
    <main className="container mx-auto max-w-3xl pt-24 p-4">
      <h1 className="text-2xl font-semibold text-white mb-4">@{params.username} • Takipçiler</h1>
      {list.length === 0 ? (
        <p className="text-slate-300">Henüz takipçi yok.</p>
      ) : (
