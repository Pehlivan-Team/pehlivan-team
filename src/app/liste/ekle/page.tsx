...
const itemSchema = z.object({
  part_name: z.string().min(3, { message: 'Parça adı en az 3 karakter olmalı.' }),
  quantity: z.coerce.number().min(1, { message: 'Miktar en az 1 olmalı.' }),
  price: z.coerce.number().min(0, { message: 'Fiyat 0 veya daha büyük olmalı.' }),
    ...
    <div className="bg-gray-900 min-h-screen text-white flex items-center justify-center py-12 px-4">
      <Card className="w-full max-w-lg bg-slate-800 border-slate-700 text-white">
        <CardHeader className="text-center">
          <CardTitle className="text-2xl font-bold">İhtiyaç Listesine Ürün Ekle</CardTitle>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <FieldGroup>
              <Field>
                ...
                <Input id="part_name" name="part_name" onChange={handleChange} />
                {errors.part_name && <FieldError>{errors.part_name}</FieldError>}
              </Field>
              <Field>
                <FieldLabel htmlFor="quantity">Adet*</FieldLabel>
                <Input id="quantity" name="quantity" type="number" onChange={handleChange} />
                {errors.quantity && <FieldError>{errors.quantity}</FieldError>}
              </Field>
              <Field>
                <FieldLabel htmlFor="price">Birim Fiyat (₺)*</FieldLabel>
                <Input id="price" name="price" type="number" step="0.01" onChange={handleChange} />
                {errors.price && <FieldError>{errors.price}</FieldError>}
              </Field>
              <Field>
                <FieldLabel htmlFor="link">Ürün Linki*</FieldLabel>
                <Input id="link" name="link" type="url" onChange={handleChange} />
