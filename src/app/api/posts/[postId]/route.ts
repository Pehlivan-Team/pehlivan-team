...
export async function PUT(req: Request, { params }: { params: { postId: string } }) {
  ...
  try {
    ...
    // Düzenlemede 'content', 'imageUrl', 'type', 'linkUrl' güncellenebilir
    const { content, imageUrl, type, linkUrl } = parsedBody.data as any
    ...
    // Admin-only: sponsored tipine güncelleme yetkisi
    if (type === 'sponsored' && !session.user.isAdmin) {
      return NextResponse.json({ error: 'Only admins can set type to sponsored' }, { status: 403 })
    }
