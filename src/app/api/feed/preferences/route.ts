...
function sanitize(input: any) {
  ...
  return {
    algorithm: ['latest', 'popular'].includes(input.algorithm) ? input.algorithm : DEFAULT_PREFERENCES.algorithm,
    showProjects: typeof input.showProjects === 'boolean' ? input.showProjects : DEFAULT_PREFERENCES.showProjects,
    showTeams: typeof input.showTeams === 'boolean' ? input.showTeams : DEFAULT_PREFERENCES.showTeams,
    showCars: typeof input.showCars === 'boolean' ? input.showCars : DEFAULT_PREFERENCES.showCars,
    hideSponsored: typeof input.hideSponsored === 'boolean' ? input.hideSponsored : DEFAULT_PREFERENCES.hideSponsored,
...
export async function GET() {
  ...
  const q = await firestoreAdmin.collection('users').where('username','==', username).limit(1).get();
  if (q.empty) {
