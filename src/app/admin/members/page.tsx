...
async function getFilteredParticipants({ page = 1, searchTerm = '', teamFilter = '' }) {
  try {
    ...
    const allTeams = Array.from(new Set(allParticipants.map((p) => p.team).filter((t) => t))).sort()
    ...
    console.error("Google Sheets'ten katılımcı verileri çekilirken hata:", error)
    return { participants: [], totalPages: 0, currentPage: 1, allTeams: [] }
  }
}
...
export default async function AdminMembersPage({
  ...
  const { participants, totalPages, currentPage, allTeams } = await getFilteredParticipants({
    page,
    searchTerm,
    teamFilter,
  })
