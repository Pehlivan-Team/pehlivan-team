...
async function getParticipantStats(): Promise<Stats> {
  try {
    ...
    const sortedByTeam = Object.entries(stats.byTeam)
      .map(([name, count]) => ({ name, count: count as number }))
      .sort((a, b) => b.count - a.count)
    const sortedByDepartment = Object.entries(stats.byDepartment)
      .map(([name, count]) => ({ name, count: count as number }))
      .sort((a, b) => b.count - a.count)
...
export default async function AdminDashboardPage() {
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-gray-400 mb-8">Aşağıda katılımcı istatistiklerini görebilirsin.</p>
