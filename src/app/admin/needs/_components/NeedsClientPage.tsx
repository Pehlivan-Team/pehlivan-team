...
export function NeedsClientPage({
  initialData,
}: {
  initialData: DepartmentNeeds[];
}) {
  const [data, setData] = useState(initialData);
  const [newCategoryName, setNewCategoryName] = useState("");
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const { data: session } = useSession();
  if (!session?.user?.permissions?.canManageNeeds) return <NoPermError />;

  const handleDelete = async (itemId: string, departmentName: string) => {
    try {
      const response = await fetch(
        `/api/admin/needs/${departmentName}/${itemId}`,
        {
          method: "DELETE",
        }
      );
      ...
      setData((currentData) =>
        currentData.map((dept) =>
          dept.departmentName === departmentName
            ? {
                ...dept,
                items: dept.items.filter((item) => item.id !== itemId),
              }
            : dept
        )
      );
  ...
  const handleAddCategory = async () => {
    ...
    try {
      ...
      setData((currentData) => [
        ...currentData,
        { departmentName: newCategoryName, items: [] },
      ]);
      setNewCategoryName("");
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              "{item.part_name}" adlı ürünü kalıcı olarak silmek
                          ...
                          <AlertDialogFooter>
                            ...
                            <AlertDialogAction
                              onClick={() =>
                                handleDelete(item.id, dept.departmentName)
                              }
