...
import { AnalyticsClientPage, AnalyticsData } from './_components/AnalyticsClientPage'
...
async function getAnalyticsData(): Promise<AnalyticsData> {
  ...
  try {
    ...
    topLinks = linksSnapshot.docs
      .map((doc) => ({
        ...(doc.data() as { slug: string; longUrl: string; clicks?: number }),
        id: doc.id,
      }))
    ...
    if (propertyId) {
      ...
      totalVisitors = parseInt(response.rows?.[0]?.metricValues?.[0]?.value || '0')
