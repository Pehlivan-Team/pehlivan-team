...
const formSchema = z.object({
  ...
  email: z.string().email({ message: 'Geçerli bir email girin.' }).optional().or(z.literal('')),
  phone: z
    .string()
    .min(10, { message: 'Telefon numarası en az 10 karakter olmalı.' })
    .optional()
    .or(z.literal('')),
  student_number: z.string().min(5, { message: 'Öğrenci numarası en az 5 karakter olmalı.' }),
    ...
    if (!parsedData.success) {
      ...
      for (const key in fieldErrors) {
        if (fieldErrors[key as keyof FormState]) {
          newErrors[key as keyof FormState] = fieldErrors[key as keyof FormState]![0]
        }
      }
    ...
    <div className="relative min-h-screen w-full flex items-center justify-center py-24 px-4">
      ...
      <Card className="relative z-6 w-full max-w-2xl bg-slate-900/80 backdrop-blur-sm border-slate-700 text-white shadow-2xl">
        <CardHeader className="text-center">
          <CardTitle className="text-2xl md:text-3xl font-bold">Topluluğumuza Katılın</CardTitle>
          <CardDescription className="text-gray-300 pt-2">
            Aşağıdaki formu doldurarak Tasarım Proje Topluluğu ailesinin bir parçası olmak için ilk
            adımı atın. Başvurunuzu heyecanla bekliyoruz!
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <FieldGroup>
              ...
              <Field>
                ...
                <Input
                  ...
                  onChange={handleChange}
                />
                {errors.phone && <FieldError className="text-red-500">{errors.phone}</FieldError>}
              </Field>
              <Field>
                <FieldLabel htmlFor="student_number">Öğrenci Numarası*</FieldLabel>
                <Input
                  type="text"
                  id="student_number"
                  name="student_number"
                ...
                {errors.student_number && (
                  <FieldError className="text-red-500">{errors.student_number}</FieldError>
                )}
              </Field>
              <Field>
                    ...
                    >
                      {formState.department ? (
                        ...
                        <div className="flex justify-between items-center">
                          <span className="text-muted-foreground">Bölüm seç</span>
                  ...
                  <PopoverContent className="w-80 max-w-none bg-gray-900">
                    <Command>
                      ...
                      <CommandList className="max-h-96 overflow-y-auto text-white scrollbar-hide">
                        ...
                        {Object.entries(departmentlist).map(([faculty, depts]) => (
                          <CommandGroup key={faculty} heading={faculty} className="text-sm">
                            {depts.map((dept) => (
                              <CommandItem
                                key={dept.key}
                                value={dept.dept}
                ...
                {errors.department && (
                  <FieldError className="text-red-500">{errors.department}</FieldError>
                )}
              </Field>
              <Field>
                <FieldLabel htmlFor="team">Hangi Takıma Katılmak İstiyorsunuz?*</FieldLabel>
                <Select
                  name="team"
                  required
                ...
                >
                  ...
                  <SelectContent className="max-h-60 overflow-y-auto bg-gray-700 text-white scrollbar-hide">
                    ...
                    <SelectItem value="Linux">Pehlivan Team / Linux Geliştirme</SelectItem>
                    <SelectItem value="Börü">Börü / Otonom Araç</SelectItem>
                    <SelectItem value="PR">PR / İçerik Üretimi</SelectItem>
                  </SelectContent>
                </Select>
                {errors.team && <FieldError className="text-red-500">{errors.team}</FieldError>}
              </Field>
              <button
                type="submit"
                disabled={isLoading}
