  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        const userRef = firestoreAdmin.collection('users').doc(credentials.email)
        ...
        // 3. Şifre doğru mu?
        const isValid = await bcrypt.compare(credentials.password, userData.hashedPassword)
  ...
  callbacks: {
    ...
    async jwt({ token, user }) {
      ...
      if (user && user.email) {
        ...
        try {
          const adminDoc = await firestoreAdmin.collection('admins').doc(userEmail).get()
